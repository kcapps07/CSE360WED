...
public class PasswordEvaluator {
	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			// valid transitions
			if (currentChar >= 'A' && currentChar <= 'Z') {  // Checks for capital letter and updates flag
				System.out.println("Upper case letter found");
				foundUpperCase = true;
			} else if (currentChar >= 'a' && currentChar <= 'z') {	//Checks for lower letter and updates flag
				System.out.println("Lower case letter found");
				foundLowerCase = true;
			} else if (currentChar >= '0' && currentChar <= '9') {	// Checks for number and updates flag
				System.out.println("Digit found");
				foundNumericDigit = true;
			} else if ("~`!@#$%^&*()_-+={}[]|\\:;\"'<>,.?/".indexOf(currentChar) >= 0) {	// Checks for special char and updates flag
				System.out.println("Special character found");
				foundSpecialChar = true;
			} else {
				passwordIndexofError = currentCharNdx;  			// Checks for invalid input character
				return "*** Error *** An invalid character has been found!";
			}
			if (currentCharNdx >= 7) {
				System.out.println("At least 8 characters found");	// Checks length meets at least 8 and updates flag
		...
		String errMessage = "";		// Outputs to user conditions not met
		if (!foundUpperCase)
